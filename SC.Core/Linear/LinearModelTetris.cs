...
namespace SC.Core.Linear
{
    /// <summary>
    /// The tetris-model builder.
    /// </summary>
    public class LinearModelTetris : LinearModelBase
    {
        /// <summary>
        ...
        public LinearModelTetris(Instance instance, Configuration config) : base(instance, config) { }

        #region Variable declaration

        private int[] _alphas = { 1, 2, 3 };
        ...
        private VariableCollection<Container, int, int, MeshCube, Piece> _lambda;
        ...
        internal override void TransformIntermediateSolution()
        {
            ...
            if (Solution == null)
            {
                Solution = Instance.CreateSolution(new Configuration() { Tetris = true });
            }
            else
            {
                Solution.Clear();
            ...
            foreach (var container in Instance.Containers)
            {
                foreach (var piece in Instance.Pieces)
                {
                    ...
                    {
                        int orientation = MeshConstants.ORIENTATIONS.Single(o => _itemOrientation[o, piece].CallbackValue > 0.5);
        ...
        internal override void TransformSolution()
        {
            ...
            foreach (var container in Instance.Containers)
            {
                foreach (var piece in Instance.Pieces)
                {
                    ...
                    {
                        int orientation = MeshConstants.ORIENTATIONS.Single(o => _itemOrientation[o, piece].Value > 0.5);
                        Solution.Add(container, piece, orientation, new MeshPoint()
                        {
        ...
        internal override void PrintSolution(TextWriter tw)
        {
            foreach (var piece in Instance.Pieces)
            {
                ...
                foreach (var cube in piece.Original.Components)
                {
                    tw.WriteLine(cube.ToIdentString());
                    foreach (var vertexID in MeshConstants.VERTEX_IDS.Skip(1))
                    {
                        tw.Write("v" + vertexID + " ");
                        foreach (var container in Instance.Containers)
                        {
                            tw.WriteLine(container.ToIdentString() + " ");
                            foreach (var gamma in MeshConstants.VERTEX_IDS.Skip(1))
                            {
                                tw.Write("v'" + gamma + "-" + _lambda[container, gamma, vertexID, cube, piece].Value.ToString(ExportationConstants.EXPORT_FORMAT_SHORT, ExportationConstants.FORMATTER) + " ");
                            }
                            tw.WriteLine();
                        }
                    }
